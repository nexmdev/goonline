...
public class DataHolder extends BaseHolder {
    ...
    public DataHolder(final View itemView) {
        ...
        this.parent = (CardView) itemView.findViewById(R.id.sub_category_other_options_parent_layout) ;
    ...
    public void bindData(ProductListing data , Context mcontext, boolean visible){
        ...
        if(visible){
            ...
            if(data.getDepartment().matches("सर्विसेस")){
                subName.setText(data.getProductName());
                subOwner.setText(data.getSellerName());
            }else{
                subName.setText(data.getProductName());
                subOwner.setText(data.getSellerName());
            ...
            subRate.setText("₹. "+priceArray[3]);
            if(data.getNoOfRaters() != 0){
                float ratings = GOApplication.round((float) data.getRatings()/data.getNoOfRaters());
